...
describe('Module 9 - worker operations', () => {
  ...
  beforeAll(async () => {
    ...
    const [adminUser, workerUser] = await Promise.all([
      prisma.user.create({
        data: {
          phoneCountryCode: '+51',
          phoneNumber: `81${stamp}`,
          passwordHash: 'test',
          fullName: `Admin M9 ${suffix}`,
          role: 'ADMIN',
          status: 'ACTIVE',
        },
      }),
      prisma.user.create({
        data: {
          phoneCountryCode: '+51',
          phoneNumber: `82${stamp}`,
          passwordHash: 'test',
          fullName: `Worker M9 ${suffix}`,
          role: 'WORKER',
          status: 'ACTIVE',
        },
      }),
  ...
  it('prevents a worker from operating another business', async () => {
    await expect(
      service.startShift(worker(), otherClubId, { deviceFingerprint: 'device-module-nine' }),
    ).rejects.toBeDefined();
  ...
  it('hides nothing through trust: backend rejects worker administration actions', async () => {
    await expect(
      service.replacePermissions(worker(), clubId, workerId, { permissions: [] }),
    ).rejects.toBeDefined();
    await expect(
      service.authorizeDevice(worker(), clubId, workerId, {
        fingerprint: 'device-module-nine',
        name: 'Android test',
        platform: 'android',
      }),
    ).rejects.toBeDefined();
  ...
  it('revokes a permission with immediate effect', async () => {
    ...
    await service.replacePermissions(admin(), clubId, workerId, {
      permissions: [WorkerPermission.VALIDATE_TICKETS],
    });
  ...
  it('authorizes a device and closes its active shift remotely', async () => {
    await service.updateWorker(admin(), clubId, workerId, {
      assignedDoor: 'Puerta 1',
      assignedZone: 'Ingreso',
      assignedPoint: 'Scanner A',
    });
    const authorized = await service.authorizeDevice(admin(), clubId, workerId, {
      fingerprint: 'device-module-nine',
      name: 'Android test',
      platform: 'android',
    });
    const started = await service.startShift(worker(), clubId, {
      deviceFingerprint: 'device-module-nine',
    });
    expect(started.shift.assignedDoor).toBe('Puerta 1');
    await expect(
      service.syncShift(worker(), clubId, started.shift.id, {
  ...
  it('records worker changes and exposes a traceable report', async () => {
    ...
    expect(
      await prisma.auditLogEntry.count({ where: { clubId, resourceType: 'WORKER_SHIFT' } }),
    ).toBeGreaterThanOrEqual(2);
    await prisma.qrValidationAttempt.deleteMany({
      where: { clubId, codeFingerprint: `m9-${suffix}` },
    });
  });
});
