...
describe('Module 10 - realtime capacity', () => {
  ...
  beforeAll(async () => {
    ...
    const [adminUser, workerUser] = await Promise.all([
      prisma.user.create({
        data: {
          phoneCountryCode: '+51',
          phoneNumber: `71${stamp}`,
          passwordHash: 'test',
          fullName: `Capacity Admin ${suffix}`,
          role: 'ADMIN',
          status: 'ACTIVE',
        },
      }),
      prisma.user.create({
        data: {
          phoneCountryCode: '+51',
          phoneNumber: `72${stamp}`,
          passwordHash: 'test',
          fullName: `Capacity Worker ${suffix}`,
          role: 'WORKER',
          status: 'ACTIVE',
        },
      }),
    ...
    const club = await prisma.club.create({
      data: { name: `Capacity Club ${suffix}`, status: 'ACTIVE' },
    });
    clubId = club.id;
    ...
    await prisma.clubWorker.create({
      data: {
        ...
        userId: workerId,
        roleLabel: 'Aforo',
        permissions: [WorkerPermission.VIEW_CAPACITY],
      },
    });
  ...
  it('starts at zero and exposes capacity only to authorized users', async () => {
    expect(await service.get(worker(), clubId, eventId)).toMatchObject({
      current: 0,
      capacity: 2,
      full: false,
    });
    await expect(
      service.get({ id: randomUUID(), role: UserRole.WORKER }, clubId, eventId),
    ).rejects.toBeDefined();
  });

  it('never exceeds the event limit under concurrent entries', async () => {
    const results = await Promise.allSettled(
      [`ticket-a-${suffix}`, `ticket-b-${suffix}`, `ticket-c-${suffix}`].map((ticketId) =>
        prisma.$transaction((tx) =>
          service.registerEntry(tx, { eventId, clubId, actorUserId: workerId, ticketId }),
        ),
      ),
    );
  ...
  it('registers exits idempotently and supports controlled reentry', async () => {
    ...
    const firstExit = await service.registerExit(
      ...
      eventId,
      ticketId,
      `exit-capacity-${suffix}`,
    );
    const repeatedExit = await service.registerExit(
      admin(),
      clubId,
      eventId,
      ticketId,
  ...
  it('requires MANAGE_CAPACITY and a bounded target for corrections', async () => {
    await expect(
      service.correct(worker(), clubId, eventId, 1, 'Conteo manual supervisado'),
    ).rejects.toBeDefined();
    await expect(
      service.correct(admin(), clubId, eventId, 3, 'Supera el máximo permitido'),
    ).rejects.toBeDefined();
    const corrected = await service.correct(
      ...
      eventId,
      1,
      'Conteo manual supervisado',
      `correction-capacity-${suffix}`,
    );
