...
export class WithdrawalsService {
  ...
  async upsertProfile(user: AuthenticatedUser, clubId: string, input: UpsertFinancialProfileDto) {
    ...
    const profile = await (this.prisma as any).clubFinancialProfile.upsert({
      ...
      create: {
        ...
        taxDocumentType: input.taxDocumentType.trim(),
        taxDocumentNumber: input.taxDocumentNumber.trim(),
        bankName: input.bankName.trim(),
        bankAccountType: input.bankAccountType.trim(),
        bankAccountEncrypted: this.encrypt(digits),
        bankAccountLast4: digits.slice(-4),
  ...
  async request(user: AuthenticatedUser, clubId: string, input: CreateWithdrawalDto) {
    ...
    const legacySplit = await this.prisma.ledgerEntry.findFirst({
      where: {
        account: { code: `CLUB:${clubId}` },
        transaction: { type: 'SALE', paymentAttempt: { provider: 'mercado_pago' } },
      },
      select: { id: true },
    });
    if (legacySplit)
      throw conflict(
        'LEGACY_SPLIT_RECONCILIATION_REQUIRED',
        'El saldo contiene ventas históricas con Split. La plataforma debe conciliarlas antes de liquidar.',
      );
    ...
    const request = await this.prisma.$transaction(async (tx) => {
      ...
      try {
        await this.ledger.moveClubFunds(tx, {
          ...
          from: 'AVAILABLE',
          to: 'HELD',
          reference: `WITHDRAWAL_HOLD:${created.id}`,
          description: 'Retención por solicitud de retiro',
        ...
        if (error instanceof Error && error.message === 'INSUFFICIENT_FINANCIAL_BALANCE') {
          throw conflict(
            ...
            'El negocio no tiene saldo disponible suficiente.',
          );
        }
        throw error;
      }
  ...
  async review(user: AuthenticatedUser, id: string, action: 'APPROVE' | 'REJECT', reason?: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!['REQUESTED', 'UNDER_REVIEW'].includes(current.status))
        throw conflict('WITHDRAWAL_NOT_REVIEWABLE', 'El retiro ya fue revisado.');
      if (action === 'REJECT') {
        if (!reason?.trim())
          throw badRequest('REJECTION_REASON_REQUIRED', 'Indica el motivo del rechazo.');
        await this.ledger.moveClubFunds(tx, {
          clubId: current.clubId,
          amountCents: current.amountCents,
          from: 'HELD',
          to: 'AVAILABLE',
          reference: `WITHDRAWAL_RELEASE:${id}`,
          description: 'Liberación por retiro rechazado',
  ...
  async markPaid(user: AuthenticatedUser, id: string, paymentReference: string, proofUrl?: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!['APPROVED', 'PROCESSING'].includes(current.status))
        throw conflict('WITHDRAWAL_NOT_PAYABLE', 'El retiro no está listo para pagarse.');
      await this.ledger.moveClubFunds(tx, {
        clubId: current.clubId,
        amountCents: current.amountCents,
        from: 'HELD',
        to: 'WITHDRAWN',
        reference: `WITHDRAWAL_PAID:${id}`,
        description: 'Retiro pagado',
        withdrawalRequestId: id,
        type: 'WITHDRAWAL',
      });
      const updated = await (tx as any).withdrawalRequest.update({
      ...
      await (tx as any).auditLogEntry.create({
        data: {
          ...
          resourceType: 'WITHDRAWAL',
          resourceId: id,
          metadata: { paymentReference, proofUrl },
        },
      });
      return updated;
    });
  ...
  async markFailed(user: AuthenticatedUser, id: string, reason: string) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (!current)
        throw notFound('WITHDRAWAL_NOT_FOUND', 'No encontramos la solicitud de retiro.');
      if (!['APPROVED', 'PROCESSING'].includes(current.status))
        throw conflict('WITHDRAWAL_NOT_FAILABLE', 'El retiro no puede marcarse como fallido.');
      await this.ledger.moveClubFunds(tx, {
        clubId: current.clubId,
        amountCents: current.amountCents,
        from: 'HELD',
        to: 'AVAILABLE',
        reference: `WITHDRAWAL_FAILED_RELEASE:${id}`,
        description: 'Liberación por retiro fallido',
        withdrawalRequestId: id,
        type: 'ADJUSTMENT',
      });
  ...
  private async assertClubAdmin(user: AuthenticatedUser, clubId: string) {
    ...
    if (user.role !== UserRole.ADMIN)
      throw forbidden('WITHDRAWAL_FORBIDDEN', 'Solo un administrador puede gestionar retiros.');
  ...
  private assertSuperAdmin(user: AuthenticatedUser) {
    if (user.role !== UserRole.SUPER_ADMIN)
      throw forbidden('WITHDRAWAL_REVIEW_FORBIDDEN', 'Solo Super Admin puede revisar retiros.');
  }
  ...
  private encrypt(value: string) {
    ...
    if (!configured && this.config.get<string>('NODE_ENV') === 'production')
      throw new Error('FINANCIAL_DATA_ENCRYPTION_KEY_REQUIRED');
    const key = createHash('sha256')
      .update(configured ?? 'beerry-dev-financial-key')
      .digest();
  ...
  private audit(
    ...
    return this.prisma.auditLogEntry.create({
      data: { actorUserId, clubId, action, resourceType, resourceId },
    });
  }
